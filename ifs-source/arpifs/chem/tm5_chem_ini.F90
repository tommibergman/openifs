...
SUBROUTINE TM5_CHEM_INI(YGFL,YDCHEM,YDCOMPO)

!**   DESCRIPTION 
!     ----------
!
!   TM5 routine for IFS chemistry: Checking tracer indices,
!      and initialization of 
!      - chem-rates lookup-table
!      - photolysis lookup-table
!      - chemistry budget preparations
...
! Some checking
 ...
 CALL IFS_INI_BUDGET(YGFL,YDCHEM)
...
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI',1,ZHOOK_HANDLE)
...
USE TM5_CHEM_MODULE    , ONLY : &
 ...
 &  INH4,     IMSA,     IMGLY,   IO3S,    IRN222,INO,      IHO2,   ICH3O2,    &
 &  IOH,      IHONO,    INO2,    INO3,    IN2O5, IHNO4,    IC2O3,  IROR,      &
 &  IRXPAR,   IXO2,     IXO2N,   INH2,    ICH3OH,IHCOOH,   IMCOOH, IC2H6,     &
...
ASSOCIATE(NCHEM=>YGFL%NCHEM, YCHEM=>YGFL%YCHEM)
   DO JL = 1,NCHEM  
     ...
     SELECT CASE ( TRIM( YCHEM(JL)%CNAME ) ) 
       ...
       CASE ('HNO4','HO2NO2')  ; LLFOUND = (IHNO4 == JL )
     ...
     IF (.NOT. LLFOUND ) THEN 
       WRITE(NULOUT,*) 'ERROR tm5_chem_ini: Wrong tracer index or status for  '//TRIM(YCHEM(JL)%CNAME)
       CALL ABOR1('tm5_chem_ini: wrong tracer index tracer name')
     ENDIF   
  ENDDO

! Also various checks in the inverse: tracers present in input table file  
! This version also requires SO3 (not OCS!) available in the list of trace gases
  LLFOUND   = .FALSE.
  DO JL = 1, NCHEM
  ...
  IF (.NOT. LLFOUND ) THEN
     CALL ABOR1('ERROR tm5_chem_ini: SO3 not defined in table-file')
  ENDIF   
...
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:TRACER_IDX_CHECK',1,ZHOOK_HANDLE)
END SUBROUTINE TRACER_IDX_CHECK


SUBROUTINE IFS_INI_BUDGET(YGFL,YDCHEM)

!**   DESCRIPTION 
!     ----------
!
!    routine for initializing the chemical budget information table
!
!
!
!**   INTERFACE.
...
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:IFS_INI_BUDGET',0,ZHOOK_HANDLE)
...
IH2O  = NCHEM+3_JPIM
  ...
  &         INO     , IHNO4    , INO2      , IHNO4    , ICH3O2  , ICH3O2NO2, IAIR     , IH2O     , IO3      , ICO       , &
  ...
  &         ISO2    , ISO3     , 1         , IHCN     , ICH3CN  , ITOL     , IO3      , INO3     , IXYL     , IO3       , &
  ...
  &         IO3     ,      0   ,     IOH ,     IOH ,     IOH ,      IOH ,    ITOL ,  ITOL  ,    IOH  ,  IXYL , &
  &         IXYL    , IAROO2   ,  IAROO2 ,  IAROO2 ,  IAROO2 ,        0 ,       0 ,     0  ,      0  ,  IHO2 , &
  &         INO     ,   IHO2   ,     INO ,     IOH ,     IOH ,      IOH ,     IOH ,    IOH ,     IOH ,  IOH  /),(/NREAC,2/))
  
  NRJ=(/ &
  &         IO3     , INO2    , IH2O2   , IHNO3  ,   IHNO4  ,     IN2O5, ICH2O, ICH2O,   ICH3O2H, INO3, &
...
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:IFS_INI_BUDGET',1,ZHOOK_HANDLE)
END SUBROUTINE IFS_INI_BUDGET





SUBROUTINE BOUNDARY_HNO3
!**   DESCRIPTION 
!     ----------
!
!    routine to read file with HNO3 boundary conditions in the stratosphere
!
!
!
!**   INTERFACE.
!     ----------
!          *BOUNDARY_HNO3* IS CALLED FROM *TM5_CHEM_INI*.
!
USE TM5_CHEM_MODULE    , ONLY : UARS_HNO3_O3
USE YOMLUN             , ONLY : NULOUT
USE PARKIND1           , ONLY : JPIM     ,JPRB
USE YOMHOOK            , ONLY : LHOOK,   DR_HOOK, JPHOOK
...
LOGICAL                        :: LLFILEOK
REAL(KIND=JPRB),DIMENSION(180*12)::ZUARS_DATA
REAL(KIND=JPHOOK)                 :: ZHOOK_HANDLE

!TM5 uars climatology file
CHARACTER(LEN=14) :: CLTM5_UARS='uars_ratio.txt'

IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:BOUNDARY_HNO3',0,ZHOOK_HANDLE)

!fill with dummy values
UARS_HNO3_O3 = - 999.9_JPRB

  LLFILEOK=.FALSE.
  INQUIRE(FILE=TRIM(CLTM5_UARS),EXIST=LLFILEOK )

  IF(.NOT. LLFILEOK)THEN
    WRITE(NULOUT,*) 'ERROR tm5_chem_ini: Cannot read TM5 file '//TRIM(CLTM5_UARS)
    CALL ABOR1('tm5_chem_ini: FILE tm5_fintwb does not exist.')
  ELSE
    WRITE(NULOUT,*) 'INFO tm5_chem_ini: OPENing TM5 file '//TRIM(CLTM5_UARS) 
    OPEN(IUTMP,FORM='FORMATTED',ACTION='READ',STATUS='OLD',   &
     & FILE=TRIM(CLTM5_UARS))
    READ(IUTMP,*) ZUARS_DATA
    CLOSE(IUTMP)
    WRITE(NULOUT,*) 'INFO TM5_chem_ini: Finished READing TM5 file '//TRIM(CLTM5_UARS) 
  ENDIF

  DO IMONTH = 1,12 
    UARS_HNO3_O3(IMONTH,:)=ZUARS_DATA(1+180*(IMONTH-1):180*(IMONTH))
  ENDDO

IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:BOUNDARY_HNO3',1,ZHOOK_HANDLE)
END SUBROUTINE BOUNDARY_HNO3


SUBROUTINE BOUNDARY_CH4STRAT
!**   DESCRIPTION 
!     ----------
!
!    routine to read file with CH4 boundary conditions in the stratosphere
!
!
!
!**   INTERFACE.
!     ----------
!          *BOUNDARY_CH4STRAT* IS CALLED FROM *TM5_CHEM_INI*.
!
USE TM5_CHEM_MODULE    , ONLY : HALOE_CH4
USE YOMLUN             , ONLY : NULOUT
USE PARKIND1           , ONLY : JPIM,JPRB
USE YOMHOOK            , ONLY : LHOOK,   DR_HOOK, JPHOOK
...
! Local parameters
INTEGER(KIND=JPIM), PARAMETER  :: IUTMP=77
INTEGER(KIND=JPIM)             :: IMONTH,ILAT
LOGICAL                        :: LLFILEOK
REAL(KIND=JPHOOK)                :: ZHOOK_HANDLE

!TM5 HALOE CH4 climatology file
CHARACTER(LEN=17) :: CLTM5_HALOE='haloe_ch4clim.dat'

IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:BOUNDARY_CH4STRAT',0,ZHOOK_HANDLE)

!fill with dummy values
HALOE_CH4 = - 999.9_JPRB

  LLFILEOK=.FALSE.
  INQUIRE(FILE=TRIM(CLTM5_HALOE),EXIST=LLFILEOK )

  IF(.NOT. LLFILEOK)THEN
    WRITE(NULOUT,*) 'ERROR tm5_chem_ini: Cannot read TM5 file '//TRIM(CLTM5_HALOE)
    CALL ABOR1('tm5_chem_ini: FILE haloe_ch4clim.dat does not exist.')
  ELSE
    WRITE(NULOUT,*) 'INFO tm5_chem_ini: opening TM5 file '//TRIM(CLTM5_HALOE) 
    OPEN(IUTMP,FORM='FORMATTED',ACTION='READ',STATUS='OLD',   &
     & FILE=TRIM(CLTM5_HALOE))
    ! Altitude 1: 45 hPa
    READ(IUTMP,*)
    DO IMONTH=1,12
      READ(IUTMP,*) (HALOE_CH4(IMONTH,ILAT,1),ILAT=1,180)
    ENDDO
    ! Altitude2: 90 hPa
    READ(IUTMP,*)
    DO IMONTH=1,12
      READ(IUTMP,*) (HALOE_CH4(IMONTH,ILAT,2),ILAT=1,180)
    ENDDO
    CLOSE(IUTMP)
    WRITE(NULOUT,*) 'INFO TM5_chem_ini: Finished reading TM5 file '//TRIM(CLTM5_HALOE) 
  ENDIF

  
  ! Conversion from ppmv to ppp
  HALOE_CH4 = HALOE_CH4*1E-6_JPRB

IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:BOUNDARY_CH4STRAT',1,ZHOOK_HANDLE)
END SUBROUTINE BOUNDARY_CH4STRAT
...
!**   INTERFACE.
!     ----------
!          *RATES* IS CALLED FROM *TM5_CHEM_INI*.
!
!      method
!      ------
...
USE TM5_CHEM_MODULE, ONLY :   KNOO3,      KHO2NO,     KMO2NO,     KNO2O3,       KNONO3,     KNO3HO2,     KN2O5A,     &
  &  KOHHONO,    KHNO4OH,     KHNO4A,     KHNO4B,     KMENO2A,    KMENO2B,      KMENO2M,    KODM,        KH2OOD,     &
  &  KO3HO2,     KN2O5B,     KHNO4A,      KHNO4B,     &
  &  KO3OH,      KHPOH,       KFRMOH,     KCH4OH,     KOHMPER,    KOHROOH,      KMO2HO2A,   KMO2HO2B,    KMO2MO2,    &
  &  KHO2OH,     KH2OH,       KC41,       KC46,       KC47A,      KC47B,        KC48,       KC61A,       KC61B,      &
  &  KC49,       KC50A,       KC50B,      KC52,       KC53,       KC54,         KC57,       KC58,        KC59,       &
  &  KC62,       KC73,        KC76,       KC77,       KC78,       KC79,         KC80,       KC81,        KC82,       &
  &  KC83,       KC84,        KC85,       KDMSOHA,    KDMSOHB,    KDMSNO3,      KNH3OH,     KNH2OH,      KNH2NO,     &
  &  KNH2NO2,    KNH2HO2,     KNH2O2,     KOHCH3OH,   KOHHCOOH,   KOHMCOOH,     KOHC2H6,    KOHETHOH,    KOHC3H8,    &
  &  KO3C3H6,    KNO3C3H6,    KOHTERP,    KO3TERP,    KNO3TERP,   KNO2OHA,      KNO2OHB ,   KOHHNO3A,    KOHHNO3B,   &
  &  KOHHNO3C,   KNO2NO3A,    KNO2NO3B,   KNO2HO2A,   KNO2HO2B,   KHO2HO2A,     KHO2HO2B,   KHO2HO2C,    KSO2OHA,    &
  &  KSO2OHB,    KDMSOHC,     KHONOA,     KHONOB,     KCOOHA,     KCOOHB,       KCOOHC,     KCOOHD,      KOHMCHO,    &
  &  KOHMCH2CHO, KNO3MCHO,    KNO3MCH2CHO,KOHMVK,     KOHOLE,     KOHMACR,      KO3MVK,     KO3OLE,      KO3MACR,    &
  &  KNO3OLE,    KOHC3H6A,    KOHC3H6B,   KOHACETA,   KOHACETB,   KACO2HO2,     KACO2MO2,   KACO2NO,     KACO2XO2,   &
  &  KXO2XO2N,   KXO2N,       KO3PO2,     KO3PO3,     KNOHYPROPO2,KHO2HYPROPO2, KNOIC3H7O2, KHO2IC3H7O2, KSO2O3G,    &
  &  KSO3H2O,    KOCSOH,      KOHHCN,     KOHCH3CN,   KOHTOL,     KO3TOL,       KNO3TOL,    KOHXYL,      KNO3XYL,    &
...
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:RATES',0,ZHOOK_HANDLE)
! start
...
DO IK=1,NTEMP
   ...
   !JEW: changed to JPL2006   
   RATES_LUT(KHO2NO,IK)=ZFARR(3.3E-12_JPRB,270._JPRB,ZTREC)       ! IUPAC 2017
   !JEW: changed to JPL2006  
   ...
   RATES_LUT(KNO2O3,IK)=ZFARR(1.4E-13_JPRB,-2470._JPRB,ZTREC)     ! IUPAC 2017
   RATES_LUT(KNONO3,IK)=ZFARR(1.8E-11_JPRB,110._JPRB,ZTREC)       ! IUPAC 2017
   ...
   !RATES_LUT(KNO2NO3A,IK)=2.4E-30_JPRB*ZT3REC**3.0_JPRB
   !RATES_LUT(KNO2NO3B,IK)=1.6E-12_JPRB*ZT3REC**(-0.1_JPRB)
   ...
   !VH 12/01/2018: change to JPL-15  
   !RATES_LUT(KN2O5,IK)=ZFARR(5.8E-27_JPRB,10840._JPRB,ZTREC)
   ...
   !RATES_LUT(KHNO4OH,IK)=ZFARR(1.3E-12_JPRB,380._JPRB,ZTREC)
...
!      
   ...
   !RATES_LUT(KC47A,IK)=3.28E-28_JPRB*ZT3REC**6.87_JPRB
   !RATES_LUT(KC47B,IK)=1.125E-11_JPRB*ZT3REC**1.105_JPRB
   ! JEW : changed to JPL 15 for consistence with HNO4, N2O5      
   ...
   !VH 12/01/2018: IUPAC (Updated PAN formation, as of 2014)   
   !RATES_LUT(KC48A,IK)=ZFARR(1.1E-5_JPRB,-10100._JPRB,ZTREC)
   !RATES_LUT(KC48B,IK)=ZFARR(1.9E17_JPRB,-14100._JPRB,ZTREC)
   !JEW Updated to make consistent..
   RATES_LUT(KC48,IK)=ZFARR(9.0E-29_JPRB,14000._JPRB,ZTREC)   
   !JEW: changed to JPL2006 
   RATES_LUT(KC49,IK)=ZFARR(2.9E-12_JPRB,500._JPRB,ZTREC)
   !RATES_LUT(KC50,IK)=ZFARR(4.3E-13_JPRB,1040._JPRB,ZTREC)
   ...
   RATES_LUT(KC78,IK)=ZFARR(2.95E-12_JPRB,-450._JPRB,ZTREC) ! IUPAC (2019 updated)
   !JEW: changed to JPL2006  
   ...
   !VH RATES_LUT(KSO3H2O,IK)=ZFARR(8.5E-41_JPRB,6540._JPRB,ZTREC)
   ...
   ! RATES_LUT(KNH2O3,IK)= ZFARR(4.3E-12_JPRB,-930._JPRB,ZTREC)!1.89e-13 at 298k
   !
   ! extended NH2 chemistry (Williams et al., 2017)
   !
   ! RATES_LUT(KNH2O2NO,IK)=ZFARR(4.0E-12_JPRB,+450._JPRB,ZTREC)!1.72e-11
   ! RATES_LUT(KNH2O2O3,IK)=ZFARR(4.3E-12_JPRB,-930._JPRB,ZTREC)!1.72e-11 
   ! RATES_LUT(KNH2O2HO2,IK)=3.4E-11_JPRB 
   ! for higher organics
   ...
   RATES_LUT(KOHC3H6B,IK) = 3.0E-11_JPRB*ZT3REC**1.0_JPRB ! Updated 
   ...
   !VH RATES_LUT(KO3TERP,IK) =  ZFARR(6.3E-16_JPRB,-580._JPRB,ZTREC) ! IUPAC
  ...
  HENRY(IALD2,IK)  = ((ZKH1+ZKH2)/2._JPRB)*1.0246_JPRB
   ! Ip et al., GRL, vol(36) 1, 2009.
   HENRY(IGLY,IK)  = 419000._JPRB*EXP(7500._JPRB*(ZTREC-1._JPRB/298.15_JPRB))
   HENRY(IHPALD1,IK)  = HENRY(IALD2,IK)
   HENRY(IHPALD2,IK)  = HENRY(IALD2,IK)  
   ...
   HENRY(IMCOOH,IK) = 4100.0_JPRB*EXP(6300._JPRB*ZTREC)*EXP(-6300._JPRB*(1._JPRB/298.15_JPRB))
...
IF (LHOOK) CALL DR_HOOK('TM5_CHEM_INI:RATES',1,ZHOOK_HANDLE)
...
FUNCTION ZFARR(PRX1,PER,PTREC)
  ...
  ! Note: these don't have KIND=JPRB
