...
SUBROUTINE WRPLFP(YDRQSP,YDRQGP,YDQTYPE,KFPRESOL,KSPEC2,KSPEC2G,KFPRGPL,KFPROMA,YDGEOMETRY,YDMODEL,YDFPFIELDS,YDOFN,PFPBUF,PSPEC,CDPREF,PTSTEP)
...
!  Gather global fields and write out
  ...
  CALL WROUTSPGB(YDMODEL,YDGEOMETRY,YDGEOMETRY%YRDIM,ZPTRFLD,IFNUM_SP,IGRIBIOSH,CDPREF,KFPRESOL,YDOFN%CSH,YDFPFIELDS,PTSTEP=PTSTEP,YDQTYPE=YDQTYPE)
...
IF (YDRQGP%NFIELDG > 0) THEN
  ...
  IF(IFNUM_GRUE > 0) THEN
    ...
    CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,KFPRGPL,IFNUM_GRUE,IGRIBIOGG,CDPREF,CLMODE,KFPRESOL,YDOFN%CUA,YDFPFIELDS,PTSTEP)
