...
SUBROUTINE WRMLFP(YDRQSP,YDRQGP,KFPRESOL,KSPEC2,KSPEC2G,KFPRGPL,KFPRGPG, &
 & KFPROMA,YDGEOMETRY,YDMODEL,YDSURF,YDFPFIELDS,YDECLDP,YDEPHY,YDRQPHY,CDCONF, &
 & LDLAG,LDOCE,YDOFN,KSTEP,PFPBUF,PTSTEP,YDQTYPE,PSPEC)
...
!  Gather global fields and write out
    ...
    CALL WROUTSPGB(YDMODEL,YDGEOMETRY,YDGEOMETRY%YRDIM,ZPTRFLD,IFNUM_SP,IGRIBIOSH,'m',KFPRESOL,YDOFN%CSH,YDFPFIELDS,PTSTEP=PTSTEP,YDQTYPE=YDQTYPE)
  ...
  IF(IFNUM_GRE > 0) THEN
    ...
    IF (IO_SERV_C001%NPROC_IO == 0) THEN 
      ...
      ELSE
        CLMODE='w'
      ENDIF
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,KFPRGPL,IFNUM_GRE,IGRIBIOGG,'s',CLMODE,KFPRESOL,YDOFN%CGG,YDFPFIELDS,PTSTEP)
    ENDIF
    LLUPD_PPT = .FALSE.
    DO JF=1,IFNUM_GRE
...
IF (IO_SERV_C001%NPROC_IO == 0) THEN
  ...
  IF (LLDYN.AND.(YDRQGP%NFIELDG) > 0 .AND. .NOT. (LDLAG.OR.LDOCE)) THEN
    ...
    IF(IFNUM_GRUE > 0) THEN
      ...
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,KFPRGPL,IFNUM_GRUE,IGRIBIOGG,'m',CLMODE,KFPRESOL,YDOFN%CUA,YDFPFIELDS,PTSTEP)
      DEALLOCATE (IGRIBIOGG)
      DEALLOCATE (ZREAL)
...
!********* this is NOT Fullpos. REK
  ...
  IF(  .NOT. (LDLAG.OR.LDOCE) ) THEN
    ...
    IF(IFNUM_GROE > 0) THEN
      ...
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,NGPTOT,IFNUM_GROE,IGRIBIOGG,'m',CLMODE,KFPRESOL,YDOFN%CGG,YDFPFIELDS,PTSTEP)
      DEALLOCATE (IGRIBIOGG)
      DEALLOCATE (ZREAL)
