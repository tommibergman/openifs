...
SUBROUTINE WRMLFP(YDRQSP,YDRQGP,KFPRESOL,KSPEC2,KSPEC2G,KFPRGPL,KFPRGPG, &
 & KFPROMA,YDGEOMETRY,YDMODEL,YDSURF,YDECLDP,YDEPHY,YDRQPHY,CDCONF, &
...
!  Gather global fields and write out
    ...
    CALL WROUTSPGB(YDMODEL,YDGEOMETRY%YRDIM,ZPTRFLD,IFNUM_SP,IGRIBIOSH,'m',KFPRESOL,YDOFN%CSH,PTSTEP)
  ...
  IF(IFNUM_GRE > 0) THEN
    ...
    IF (IO_SERV_C001%NPROC_IO == 0) THEN 
      ...
      ELSE
        CLMODE='w'
      ENDIF
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,KFPRGPL,IFNUM_GRE,IGRIBIOGG,'s',CLMODE,KFPRESOL,YDOFN%CGG,PTSTEP)
    ENDIF
    LLUPD_PPT = .FALSE.
    DO JF=1,IFNUM_GRE
...
IF (IO_SERV_C001%NPROC_IO == 0) THEN
  ...
  IF (LLDYN.AND.(YDRQGP%NFIELDG) > 0 .AND. .NOT. (LDLAG.OR.LDOCE)) THEN
    ...
    IF(IFNUM_GRUE > 0) THEN
      ...
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,KFPRGPL,IFNUM_GRUE,IGRIBIOGG,'m',CLMODE,KFPRESOL,YDOFN%CUA,PTSTEP)
      DEALLOCATE (IGRIBIOGG)
      DEALLOCATE (ZREAL)
...
!********* this is NOT Fullpos. REK
  ...
  IF(  .NOT. (LDLAG.OR.LDOCE) ) THEN
    ...
    IF(IFNUM_GROE > 0) THEN
      ...
      CALL WROUTGPGB(YDMODEL,YDGEOMETRY,ZREAL,NGPTOT,IFNUM_GROE,IGRIBIOGG,'m',CLMODE,KFPRESOL,YDOFN%CGG,PTSTEP)
      DEALLOCATE (IGRIBIOGG)
      DEALLOCATE (ZREAL)
