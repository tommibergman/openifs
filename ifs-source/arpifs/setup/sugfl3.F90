...
ASSOCIATE(YDDYN=>YDMODEL%YRML_DYN%YRDYN,YGFL=>YDMODEL%YRML_GCONF%YGFL,YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY, &
 & YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY,YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL,YDDYNA=>YDMODEL%YRML_DYN%YRDYNA,YDCOMPO=>YDMODEL%YRML_CHEM%YRCOMPO)
ASSOCIATE(LSPLTHOIGFL=>YDDYN%LSPLTHOIGFL,LPC_FULL=>YDDYNA%LPC_FULL,LPC_CHEAP=>YDDYNA%LPC_CHEAP, &
 ...
 & YCDNC_NL=>YGFL%YCDNC_NL, YICNC_NL=>YGFL%YICNC_NL, YRE_LIQ_NL=>YGFL%YRE_LIQ_NL, YRE_ICE_NL=>YGFL%YRE_ICE_NL, & !eehol: add cloud prop
 ...
 & YCDNC=>YGFL%YCDNC, YICNC=>YGFL%YICNC, YRE_LIQ=>YGFL%YRE_LIQ, YRE_ICE=>YGFL%YRE_ICE, & !eehol: add cloud prop
 ...
 & LAERAOT=>YGFL%LAERAOT,LAERLISI=>YGFL%LAERLISI,YPHYCTY=>YGFL%YPHYCTY,YPHYCTY_NL=>YGFL%YPHYCTY_NL,AERO_SCHEME=>YDCOMPO%AERO_SCHEME)
!     ------------------------------------------------------------------
...
IF (TRIM(AERO_SCHEME)=="hamm7" )THEN
    ...
    END IF
    IF (YICNC%LACTIVE) THEN
      CALL SET_GFL_ATTR(YDDIMV, YGFL, YICNC, LDADV=.FALSE., LDT9=.FALSE., LDPHY=.FALSE.) 
    END IF
    IF (YRE_LIQ%LACTIVE) THEN
      CALL SET_GFL_ATTR(YDDIMV, YGFL, YRE_LIQ, LDADV=.FALSE., LDT9=.FALSE., LDPHY=.FALSE.) 
    END IF
    IF (YRE_ICE%LACTIVE) THEN
      CALL SET_GFL_ATTR(YDDIMV, YGFL, YRE_ICE, LDADV=.FALSE., LDT9=.FALSE., LDPHY=.FALSE.) 
    END IF
END IF
