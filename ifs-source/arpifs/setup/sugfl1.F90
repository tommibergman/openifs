...
LOGICAL, POINTER :: LCHEM_DIA, LCHEM_DDFLX, LCOMPO_DDFLX_DIR, LCHEM_TROPO, &
 ...
 & LCOMPO_DCDD,  & 
 & LAERNITRATE, LAEREQSAM4CLIM, LAERSOA, LINJ, LAEROSFC, LAERSOA_COUPLED, LCHEM_REVCHEM, LAERRESUSPENSION, LAERNUCL, &
 & LCHEM_ARPCLIM

REAL(KIND=JPRB), POINTER :: RCHEM_DIA_PERIOD
 
CHARACTER(LEN=20) :: CHEM_SCHEME, CHEM_LCOVERS, AERO_SCHEME
INTEGER(KIND=JPIM), POINTER :: NCHEM_LCOCOEF, KCHEM_SOLVE, KCHEM_YEARPI, KCHEM_NOXADV, KCHEM_WETDEP, KCHEM_DRYDEP, KGHG_CHEMTEND_CH4
...
! pointers for HAM-M7CTL 
INTEGER(KIND=JPIM), POINTER :: NWATER, NSNUCL, NONUCL
...
TYPE(TYPE_GFL_NAML) :: &
 ...
 & YCDNC_NL, YICNC_NL, YRE_LIQ_NL, YRE_ICE_NL, & !eehol: added cloud prop
...
ASSOCIATE(YGFL=>YDMODEL%YRML_GCONF%YGFL,YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY, YDCOMPO=>YDMODEL%YRML_CHEM%YRCOMPO, YDCHEM=>YDMODEL%YRML_CHEM%YRCHEM)
...
YCDNC_NL       => YGFL%YCDNC_NL !eehol: CDNC
YICNC_NL       => YGFL%YICNC_NL !eehol: ICNC
YRE_LIQ_NL     => YGFL%YRE_LIQ_NL !eehol: liq eff rad
YRE_ICE_NL     => YGFL%YRE_ICE_NL !eehol: ice eff rad
...
IF (NCHEM+NGHG+NAERO > 0 ) THEN 
  ...
  IF (trim(AERO_SCHEME) == 'hamm7' ) THEN 
    CALL POSNAM(NULNAM,'NAMM7CTL') !Read only if AERO_SCHEME=hamm7
    READ(NULNAM,NAMM7CTL)
  ENDIF
