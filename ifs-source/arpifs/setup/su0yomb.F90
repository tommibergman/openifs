...
IF ( NCONF == 701 ) THEN
  ...
  CALL ABOR1('FC-Only/OIFS - CALL SUALCOS, SUALGES and SUELGES in su0yomb should never be called - Exit')
...
!     Setup cost function arrays
  ...
  CALL ABOR1('FC-only/OIFS - CALL Sualcos in su0yomb should never be called - Exit')
...
! Fields for physics
  ...
  CALL SUMCCLAG(YDGEM,YDMODEL%YRML_GCONF,YDMODEL%YRML_AOC,YDMODEL%YRML_CHEM%YRCOMPO, &
       ...
       &  YDMODEL%YRML_PHY_EC%YREPHY, NULOUT, YDSURF=YDSURF, NFLEVG=YDDIMV%NFLEVG)
ENDIF
...
IF (NFPOS /= 0) THEN
  ...
  IF (NGRIBFILE==1 .OR..NOT.LARPEGEF) THEN
    IF (LGRIB_API) THEN
      ...
      CALL SU_GRIB_API(YDGEOMETRY,YDMODEL,YDFPOS%YFPVAB,YDMODEL%YRML_AOC%YRMCC%LMCC04,YGBH, &
...
ELSE
  ...
  IF (LGRIB_API) THEN
    ...
    CALL SU_GRIB_API(YDGEOMETRY,YDMODEL,YDGEOMETRY%YRVAB,YDMODEL%YRML_AOC%YRMCC%LMCC04,YGBH)
  ELSE
