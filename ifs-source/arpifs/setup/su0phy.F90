...
!                               change default value to .TRUE. for LNODIFQC (from JF Guérémy)
...
!      F. Vana   11-Sep-2020: SLAVEPP and progn. cloud in simpl physics
!      J. McNorton (Apr 2021): Added LEURBAN switch
!      R. Hogan  (Jun 2021): Turned on HLO ozone by default
...
INTEGER(KIND=JPIM), POINTER :: NLWFR, NLOOPLW, NCO2CLIMYY1, NCO2CLIMYY2, &
 ...
 & NALBEDOSCHEME, NEMISSSCHEME

LOGICAL, POINTER :: LWLCLHR, LOPTLWPR, LETRAJP, LETRAJPT, LERADI2, LERADS2, &
 & LERADSW2, LERADLW2, LERADN2, LERADFL2, LEDCLD2, LENCLD2, LEPCLD2, LEVAPLS2, &
 ...
 & LEEVAP, LEGWDG, LEOZOC, LEQNGT, LERADI, LERADIMPL, LERADS, LESICE, LESURF, LEGWWMS, &
 & LEVDIF, LEVDIFSL, LEVGEN, LESSRO, LELAIV, LESN09, LESNML, LEURBAN, LECTESSEL, &
 & LEAGS,  LEFARQUHAR, LEOPTSURF, LEC4MAP, LEAIRCO2COUP, LEFLAKE, &
 & LEPCLD, LAGPHY, LEO3CH, LO3CH_SAFE, LO3CH_OLDVER, LO3CH_BMS, LO3CH_HLO, LNEEONLINE, LWETONLINE, LBFASCO2, &
 & LWCOU, LWCOU2W, LWCUR, LWCOURNW, LWCOUHMF, LWSTOKES, &
 & LBUD23, LPPTILES, LBUDCYCLE, LEXTRATEND, LEMETHOX, LERAIN, LVDFTRAC, LVDFNEWTRAC, LMFTRAC, & 
 & LMFSCAV, LEOCWA, LEOCCO, LEOCML, LEOCLAKE, LEOBC, LEOBCICE, LEOLAKESST, &
 & LOCMLTKE, LEOCSA, LEOCLA, LECURR, LEMWAVE, LESURFTRAJ, &
 & LECLIPQT0, LECLIPCLDT0, LDUCTDIA, LEGBRAD, LERAINGG, LESMOS, LESMAP, INVML, &
 & LSPHLC, LVDFLC, LSDRLC, LZMCON, LKEXP, LVDFDS, LSDRDS, LREGWWMS, LREGSF, LVEXTRDYNACC, &
 & LNCLIN, LREGCL, LRCOEF, LTLADDIA, LGLOBRAD, LELIGHT, LEOBCMAX, LRAD_CLOUD_INHOMOG, LDIAGTURB_EC, LDIAGTURBGRAD_EC, &
 & LRDALB
...
ASSOCIATE(NSURFEXCTLMAX=>YDMSE%NSURFEXCTLMAX, NSURFEXCTL=>YDMSE%NSURFEXCTL, &
...
LDIAGTURBGRAD_EC=> YDEPHY%LDIAGTURBGRAD_EC
LVEXTRDYNACC    => YDEPHY%LVEXTRDYNACC
...
!   impact of moving from 0.835 to 0.8 here is a warming of several
!   degrees in 2-m temperature over Antarctica and Greenland.
!   We use an intermediate value of 0.82;
...
!     Switch off MF physics in AROME forecast:
      ...
      LRCO2=.FALSE.
      NPHY = JPHYARO
      IF (LSCREEN .OR. (NCONF == 131) .OR. LOBSC1) THEN
        LSOLV=.TRUE.
        LFGEL=.TRUE.
      ENDIF
    ENDIF
...
IF(LMSE .AND. .NOT.(LSCREEN .OR. (NCONF == 131) .OR. LOBSC1)) THEN
  LSOLV=.FALSE.
  LFGEL=.FALSE.
  LSNV=.FALSE.
  LVGSN=.FALSE.
...
ENDIF
IF(.NOT.YDDYNA%LSLAG .AND. LSLPHY) THEN
  LSLPHY=.FALSE.
  WRITE(NULOUT,'('' INFO - LSLPHY FORCED TO .FALSE. '')')
ENDIF
...
ENDIF
IF(LERADIMPL.AND.(.NOT.LAGPHY)) THEN
  LERADIMPL=.false.
  WRITE(NULOUT,'('' WARNING - LERADIMPL RESET TO .F. '')')
ENDIF
...
IF(LWCOU2W .AND. LWCOU) THEN
  ...
  IF(LOCMLTKE) LWSTOKES=.TRUE.  ! Stokes drift is also needed when LOCMLTKE is true 

ENDIF

IF(.NOT. LWCOU2W .OR. .NOT. LWCOU) THEN
  IF(LWCOUHMF) WRITE(NULOUT,'('' WARNING LWCOUHMF TRUE -> LWCOUHMF RESET TO .FALSE. '')')
  LWCOUHMF=.FALSE.
ENDIF

IF (LOCMLTKE) THEN
  IF(LEOCWA) WRITE(NULOUT,'('' WARNING LOCMLTKE TRUE -> LEOCWA RESET TO .FALSE. '')')
  LEOCWA=.FALSE.
  IF(LEOCML) WRITE(NULOUT,'('' WARNING LOCMLTKE TRUE -> LEOCML RESET TO .FALSE. '')')
  LEOCML=.FALSE.
ENDIF
...
ENDIF
IF (LCOEFK_TOMS.AND.( (TRIM(CGMIXLEN) /= 'EL0').AND.&
   & (TRIM(CGMIXLEN) /= 'EL1').AND.(TRIM(CGMIXLEN) /= 'EL2').AND.&
   & (TRIM(CGMIXLEN) /= 'EL3').AND.(TRIM(CGMIXLEN) /= 'EL4').AND.&
   & (TRIM(CGMIXLEN) /= 'EL5') )) THEN
  ! * TOM terms can be computed only with a TKE mixing length
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SU0PHY: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,'(1X,A)')&
   & ' SU0PHY: TOMs can be activated only with TKE-type mixing lengths or EL0 '
ENDIF
...
WRITE(UNIT=KULOUT,FMT='('' LCONDWT='',L5,'' LCVPP = '',L5,&
 ...
 & '' LCOEFK_RIS= '',L5,'' LCOEFK_RIH= '',L5,'' LCOEFK_F1= '',L5,&
 ...
 & LCOEFK_PTTE,LCOEFK_MSC,LCOEFK_RIS, LCOEFK_RIH,LCOEFK_F1,LCOEFK_FLX,&
...
WRITE(UNIT=KULOUT,FMT='('' LEDCLD = '',L5,'' LEGWDG = '',L5&
 ...
 & ,'' LBUDCYCLE = '',L5,'' LMFSCAV = '',L5,'' LNEEONLINE = '',L5,'' LWETLONLINE = '',L5&
 & ,'' LBFASCO2 = '',L5&
 & ,'' LEO3CH = '',L5,'' LO3CH_SAFE = '',L5,'' LO3CH_OLDVER = '',L5&
 & ,'' LO3CH_HLO = '',L5,'' CO3CHEM = '',A4,'' LDIAGTURB_EC = '',L5)')&
 & LEDCLD,LEGWDG,LEGWWMS,LEOZOC,LEQNGT,LBUDCYCLE,LMFSCAV,LNEEONLINE,LWETONLINE,&
 & LBFASCO2,LEO3CH,LO3CH_SAFE,LO3CH_OLDVER,LO3CH_HLO,CO3CHEM,LDIAGTURB_EC
WRITE(UNIT=KULOUT,FMT='('' LERADI = '',L5,'' LERADIMPL = '',L5&
 & ,'' LESURF = '',L5,'' LEVDIF = '',L5,'' LEVDIFSL = '',L5&
 & ,'' LEOCWA = '',L5,'' LEOCCO = '',L5,'' LEOCSA = '',L5&
 & ,'' LEOCLA = '',L5,'' LECURR = '',L5,'' LESURFTRAJ = '',L5,'' RTHRFRTI = '',F7.2&
 & ,'' RCIMIN = '',F7.2, '' LESICE = '',L5&
 & )')&
 & LERADI,LERADIMPL,LESURF,LEVDIF,LEVDIFSL,LEOCWA,LEOCCO,LEOCSA,LEOCLA,LECURR,LESURFTRAJ,&
 & RTHRFRTI,RCIMIN,LESICE
WRITE(UNIT=KULOUT,FMT='('' LEPCLD = '',L5,'' LEMETHOX= '',L5&
 & ,'' LEVGEN = '',L5,'' LESSRO = '',L5&
 & ,'' LELAIV = '',L5,'' LECTESSEL = '',L5&
 & ,'' LEAGS  = '',L5,'' LEFARQUHAR  = '',L5&
 & ,'' LEOPTSURF  = '',L5&
 & ,'' LEFLAKE = '',L5,'' LEOCML = '',L5,'' LEOBC = '',L5&
 & ,'' LEC4MAP = '',L5,'' LEAIRCO2COUP = '',L5&
 & ,'' LEOBCICE = '',L5,'' LOCMLTKE = '',L5&
 & ,'' LESN09 = '',L5,'' LESNML = '',L5,'' LEURBAN = '',L5,'' LDUCTDIA = '',L5,'' RLAIINT = '',F7.2&
 & ,'' LEOCLAKE = '',L5,'' LEOLAKESST= '',L5,'' LELIGHT= '',L5,'' NLIMODE = '',I3&
 & ,'' NSNMLWS = '',I3,'' LRDALB= '',L5 &
 & )')&
 & LEPCLD,LEMETHOX,LEVGEN,LESSRO,LELAIV,LECTESSEL,LEAGS,LEFARQUHAR,LEOPTSURF,&
 & LEC4MAP,LEAIRCO2COUP,&
 & LEFLAKE,LEOCML,LEOBC,LEOBCICE,LOCMLTKE,&
 & LESN09,LESNML,LEURBAN,LDUCTDIA,RLAIINT,LEOCLAKE,LEOLAKESST,LELIGHT,NLIMODE,NSNMLWS,LRDALB
...
IF (LETRAJP) THEN
  ...
  WRITE(UNIT=KULOUT,FMT='('' LEDCLD2 = '',L4&
   ...
   & ,'' LNCLIN  = '',L4&
   & )')&
   & LEDCLD2,LENCLD2,LEPCLD2,LEVAPLS2,LREGCL,LNCLIN
...
ELSE
  ...
  WRITE(UNIT=KULOUT,FMT='('' LEPCLD2 = '',L4)')LEPCLD2
