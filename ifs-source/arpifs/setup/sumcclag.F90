...
SUBROUTINE SUMCCLAG(YDGEM,YDML_GCONF,YDML_AOC,YDCOMPO,YDCHEM,YDEAERSRC,YDEPHY,KULOUT,YDSURF, NFLEVG)
...
INTEGER(KIND=JPIM)                   , OPTIONAL, INTENT(IN) :: NFLEVG
INTEGER(KIND=JPIM)               :: IOPROC, ISTEP, ITIM, IT, IC, ICHEMFLX,ITIM_AERCLIM
...
ASSOCIATE(&
  ...
  & LMCCIEC_AERCLIM   => YDML_AOC%YRMCC%LMCCIEC_AERCLIM, &
  ...
  & NSO4SCHEME   => YDEAERSRC%NSO4SCHEME,        NYSDMP_AERCLIM  => YDML_AOC%YRMCC%NYSDMP_AERCLIM,&
  & NPAERCLIM_1  => YDML_AOC%YRMCC%NPAERCLIM_1,           NPAERCLIM_2     => YDML_AOC%YRMCC%NPAERCLIM_2, &
  & NCLIGC_AERCLIM=>YDML_AOC%YRMCC%NCLIGC_AERCLIM,        NAEROCLIM        => YDML_GCONF%YGFL%NAEROCLIM, &
  & YAEROCLIM_NL   =>YDML_GCONF%YGFL%YAEROCLIM_NL)
...
IF (LMCCEC) THEN  !BOUNDARY CONDITIONS UPDATED BY ECMWF ROUTINES
   ...
   IF (ECE_CLIMR) THEN  ! EC-Earth-style reading of CLIMR file
        ...
        NCLIGC(6)=66
        NCLIGC(7)=67
   ELSEIF (LMCC04.AND.(.NOT.LNEMO1WAY)) THEN !OASIS COUPLER ACTIVE IN ECMWF ROUTINES
#else
...
! Allocates the necessary space
  ...
  IF (LMCC_COMPO) THEN
    ...
    IF( NSO4SCHEME == 2 ) THEN
      ...
      WRITE(KULOUT,"(1X,A20)")  ' EXPECTED ORDER '
      DO IT=1,NAEROCLIM 
         WRITE(KULOUT,"(1X,3I10)") IT,  NCLIGC_AERCLIM(IT),  NYSDMP_AERCLIM(IT)
      ENDDO
