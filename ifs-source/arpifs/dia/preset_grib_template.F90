...
SUBROUTINE PRESET_GRIB_TEMPLATE(YDGEOMETRY,YDMODEL,YDVAB,LDMCC04,CDTYPE,YDGBH,YDFPUSERGEO)
...
INTEGER(KIND=JPIM) :: IJOUR, IMOIS, ILMOIS(12), IAN, ICNDAT, NSTADD, NSTASS
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP,  &
 & YDCSGLEG=>YDGEOMETRY%YRCSGLEG, YDVETA=>YDGEOMETRY%YRVETA, YDVFE=>YDGEOMETRY%YRVFE,  &
 ...
 & YDCSGEOM=>YDGEOMETRY%YRCSGEOM, &
  ...
  & YDSPGEOM=>YDGEOMETRY%YSPGEOM, YDRIP=>YDMODEL%YRML_GCONF%YRRIP)
...
CASE('GG')
  IF( NSMAX >= 7999 ) THEN
    CALL IGRIB_CLONE(YGRIB_SAMPLES%NGRIB_HANDLE_GG2,YDGBH%NGRIB_HANDLE_GG)
  ELSE
...
CASE('DIAG')
  CALL IGRIB_CLONE(YGRIB_SAMPLES%NGRIB_HANDLE_DIAG,YDGBH%NGRIB_HANDLE_DIAG)
  IGRIB_HANDLE = YDGBH%NGRIB_HANDLE_DIAG
CASE DEFAULT
...
CALL UPDCAL(IDD, IMM, IYYYY, NSTADD, IJOUR, IMOIS, IAN, ILMOIS, NULOUT)

! Time in seconds since date above (time of day in seconds)
ITIME = ITIME - NSTADD*24*JPSHOUR
ICNDAT = IAN*10000+IMOIS*100+IJOUR

WRITE(NULOUT,FMT='(''PRESET_GRIB_TEMPLATE: WRITE REFERENCE DATE AND TIME'',I10,I10)') ICNDAT,ITIME
...
CALL IGRIB_SET_VALUE(IGRIB_HANDLE,'dataDate',ICNDAT)
