...
SUBROUTINE WROUTGPGB(YDGEOMETRY,PGGFLD,KGPTOT,KGGMAX,KGRIBIO,CDLEVTYPE,CDMODE,KRESOL,CDFN,PTSTEP)
...
USE YOMVAR   , ONLY : LTWINC, NSIM4D, MBGVEC, LTWBGV
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,TSTEP=>YRRIP%TSTEP, NYMLPPGP=>YRRIP%NYMLPPGP)
!     ------------------------------------------------------------------
...
IF(NOUTTYPE /= 2) THEN  ! Not writing to FDB
  ...
  ELSE
    IF(LTWINC) THEN
      ...
      WRITE(CLFNGG,'(A4,A4,I3.3,''+'',I6.6)') CLR1INC,CLR2,NSIM4D,IINC
    ELSE
...
LLFPDIST=LECFPOS .AND. CFPFMT == 'GAUSS' 
...
ELSE
  ...
  IF (LXIOS) THEN
    ...
    CALL XIOS_PUT(YDGEOMETRY,YL_IOREQUEST,CMODE=CLMODE,PR3=ZGGFLD)
  ELSE
    CALL SETUP_IOSTREAM(YL_IOSTREAM,'CIO',TRIM(CLFN),CDMODE=CLMODE,KIOMASTER=1)
    CALL IO_PUT(YDDIM,YL_IOSTREAM,YL_IOREQUEST,PR3=ZGGFLD)
