SUBROUTINE ECE_SUECOZV_CMIP6(KINDAT, KMINUT, YDERDI)
...
!---------------------------------------------------------------------
  ...
  USE ECE_CMIP, ONLY: CMIP6DATADIR, CMIP7DATADIR, SCENARIONAME, NCMIPFIXYR

  IMPLICIT NONE

  INTEGER(KIND=JPIM), INTENT(IN)    :: KINDAT
  INTEGER(KIND=JPIB), INTENT(IN)    :: KMINUT
  TYPE(TERDI),        INTENT(IN)    :: YDERDI ! Output gas concentrations
!     -----------------------------------------------------------------
...
! SET TIME INTERVAL
PRINT*,"ECE_SUECOZV_CMIP6: IYR, NCMIPFIXYR = ", IYR, NCMIPFIXYR
...
! limit IYR to available dataset
  ...
  IF (IYR /= IYROLD) THEN
    ...
    IF (MYPROC == RPRC) THEN
      ...
      PRINT*, "ECE_SUECOZV_CMIP6: DIR, FILE ", CMIP6DATADIR, CO3DATAFIL 
...
!*( Field is also transformed in kg/kg! )
    DO JK = 1, NLV1
      DO JL = 1, NLAT1
        DO JI = 1, NLON1
          ROZT1(JI, JL, JK) = YDERDI%RO3*(ZOZCL(JI, JL, JK, IM1)&
