SUBROUTINE ECE_UPDCLIE_CPL(YDGEOMETRY, YDSURF, YDMCC, YDDYNA, YDRIP, PTSTEP)
    ...
    USE ECEARTH
    USE CPLNG2
    USE SURFECE, ONLY: SURFECE_SET_LANDICE_LOCAL
...
#include "surf_inq.h"
    ...
    REAL(KIND=JPRB),POINTER :: CPL_FLD_PLIT(:)
    REAL(KIND=JPRB), ALLOCATABLE :: ZLOC_PLIT(:)
    REAL(KIND=JPRB), ALLOCATABLE :: ZLOC_PLIT_B(:,:)
    ...
    IF (ECE_CPL_ISMM) THEN
      CALL CPLNG2_EXCHANGE(INT(PTSTEP,KIND=JPIM), ECE_CPL_STAGE_ISM_RCV, YDDYNA, YDRIP)

      ! Update LANDICE mask from OASIS-delivered per-region PLIT fields
      IF (ECE_LANDICE) THEN
        ALLOCATE(ZLOC_PLIT(NGPTOT))
        ZLOC_PLIT(:) = 0._JPRB
        DO II = 1, NISMRGNS
          CPL_FLD_PLIT => CPLNG2_FLD(CPLNG2_IDX('PLIT_'//ISMRGNPFX(II)))%D(:,1,1)
          ZLOC_PLIT(:) = ZLOC_PLIT(:) + CPL_FLD_PLIT(:)
        ENDDO
