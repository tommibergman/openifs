SUBROUTINE ECE_PISCES_SET_STATE(KDIM,YDMODEL,PGFL)
...
#include "update_fields.intfb.h"
    ...
    IF (LHOOK) CALL DR_HOOK('ECE_PISCES_SET_STATE',0,ZHOOK_HANDLE)
    ASSOCIATE( &
    ...
    ! =========================================================================
    ! *** CO2 concentration from lowest model level to ocean BGC
    ! =========================================================================
    ...
    ! Send CO2 from lowest model level (level KDIM%KLEV) to ocean via coupling
    ! convert from mmr to ppm
    CPLNG2_FLD(CPLNG2_IDX('A_XCO2_oce'))%D(IG:IG+IE,1,1) = PGFL(IL:IL+IE,KDIM%KLEV,ICO2_INDEX) * RMD / RMCO2 * (10**6)

    END ASSOCIATE
    IF (LHOOK) CALL DR_HOOK('ECE_PISCES_SET_STATE',1,ZHOOK_HANDLE)

#endif

END SUBROUTINE ECE_PISCES_SET_STATE
