SUBROUTINE ECE_RECOM_SET_STATE(KDIM,YDMODEL,PGFL)
...
#include "update_fields.intfb.h"
    ...
    IF (LHOOK) CALL DR_HOOK('ECE_RECOM_SET_STATE',0,ZHOOK_HANDLE)
    ASSOCIATE( &
    ...
    CPLNG2_FLD(CPLNG2_IDX('A_XCO2_oce'))%D(IG:IG+IE,1,1) = PGFL(IL:IL+IE,KDIM%KLEV,ICO2_INDEX)

    END ASSOCIATE
    IF (LHOOK) CALL DR_HOOK('ECE_RECOM_SET_STATE',1,ZHOOK_HANDLE)

#endif

END SUBROUTINE ECE_RECOM_SET_STATE
