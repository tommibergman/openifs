SUBROUTINE ECE_FESIM_GET_ICE_STATE(KSTGLO,KIDIA,KFDIA,SNOW_THICKNESS,ICE_ALBEDO)
    ...
    IF(PRESENT(SNOW_THICKNESS)) SNOW_THICKNESS(:) = CPLNG_FLD(CPLNG_IDX('A_Snow_thickness'))%D(IG:IG+IE,1,1)
    IF(PRESENT(ICE_ALBEDO)) ICE_ALBEDO(:) = CPLNG_FLD(CPLNG_IDX('A_Ice_albedo'))%D(IG:IG+IE,1,1)
