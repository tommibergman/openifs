...
IF (NACTAERO /= 0 .AND. LL_HRES) THEN
  IF (.NOT.LAERINIT) THEN
    ...
    CALL AERINI_LAYER( YDGEOMETRY, YDSURF,   YDMODEL, KDIM,  PAUX, STATE_TMP, &
                     & YDVARS%R,   YDVARS%S, PSURF,   SURFL, GEMSL, PAUX%PGAW)
  ELSE
...
IF (ECE_CPL_FESOM_RECOM) CALL ECE_RECOM_SET_STATE(KDIM,YDMODEL,PGFL)
IF (ECE_CPL_NEMO_PISCES) CALL ECE_PISCES_SET_STATE(KDIM,YDMODEL,PGFL)
#endif
