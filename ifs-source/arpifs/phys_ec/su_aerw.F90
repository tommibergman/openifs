...
REAL(KIND=JPRB), POINTER :: RAERDUB, RDDUAER(:), RFCTDUR, RFCTSSR, RLATVOL,&
 ...
 & RAERVOLC(:,:), RAERVOLE(:,:), RVOLERUZ(:), DCAL, RSIGMA_W, RMINCDNC
...
RSIGMA_W  => YDEAERATM%RSIGMA_W
RMINCDNC  => YDEAERATM%RMINCDNC
...
RSIGMA_W = 0.8_JPRB ! standard deviation of updraft velocity pdf
RMINCDNC = 10.0_JPRB! minimum CDNC in cloud [# cm-3]
...
!              -------------------
  ...
  WRITE(UNIT=NULOUT,FMT='('' RSIGMA_W = '',F6.3)') RSIGMA_W
  WRITE(UNIT=NULOUT,FMT='('' RMINCDNC = '',F6.3)') RMINCDNC
