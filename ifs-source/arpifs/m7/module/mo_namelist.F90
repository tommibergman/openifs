...
!gf            CALL message('', 'Namelist file '//TRIM(file)//' reopened')
            WRITE(*,*) 'Namelist file --> '//TRIM(file)//' --> reopened'
          ELSE
!gf            CALL message('', 'Namelist file '//TRIM(file)//' already opened, rewind file to beginning')
            WRITE(*,*) 'Namelist file --> '//TRIM(file)//' --> already opened, rewind file to beginning'
    ...
    IF (istat == 0) THEN
      ...
      unit = top
!gf      CALL message('', 'Opened namelist file '//TRIM(file))
      WRITE(*,*) 'Opened namelist file --> '//TRIM(file)
    ELSE
!gf
!      CALL finish('open_nml','Could not open namelist file '//TRIM(file))
      WRITE(*,*) 'open_nml:Could not open namelist file --> '//TRIM(file)
      STOP
!gf
    ENDIF
...
!      CALL finish('position_nml','Namelist file '//TRIM(namelist_file_list(unit)%filename)//' already closed')
      WRITE(*,*) 'position_nml:Namelist file --> '//TRIM(namelist_file_list(unit)%filename)//' --> already closed'
      STOP
!gf
...
!      CALL finish('position_nml',message_text)
      WRITE(*,*) 'Namelist file for handler --> ', unit, ' --> has not been opened'
      STOP
!gf
    ELSE
    ...
    IF (PRESENT (status)) status = stat
      ...
      END SELECT
!gf     CALL message ('position_nml','File '//TRIM(yfilename)// 'namelist /'//TRIM(test)//'/ '//TRIM(action))
     WRITE(*,*) 'position_nml: File --> '//TRIM(yfilename)// 'namelist /'//TRIM(test)//'/ '//TRIM(action)
  ...
  SUBROUTINE close_nml (unit)
    ...
    INTEGER :: istat
    IF (unit == -1) THEN
!gf      CALL finish('close_nml','Namelist file not opened')
      WRITE(*,*) 'close_nml: Namelist file not opened'
    ELSE
      IF (namelist_file_list(unit)%fileunit == -1) THEN
!gf        CALL finish ('close_nml','Namelist file '//TRIM(namelist_file_list(unit)%filename)//' already closed')
        WRITE(*,*) 'close_nml: Namelist file --> '//TRIM(namelist_file_list(unit)%filename)//' --> already closed'
      ENDIF
    ENDIF
...
!      CALL finish('close_nml','Could not close namelist file '//TRIM(namelist_file_list(unit)%filename))
      WRITE(*,*) 'close_nml:Could not close namelist file --> '//TRIM(namelist_file_list(unit)%filename)
      STOP
!gf
    ENDIF
