...
MODULE mo_netcdf
  ...
  INTEGER :: CANOPY     !=  73
  INTEGER :: COSP_LEVELS!= 103 !ZK #474
...
CONTAINS
  ...
  SUBROUTINE IO_init_dims
    ...
    USE mo_cosp_constants, ONLY: Nlr !ZK #474
...
!vg                                                    698.0_dp/))
    ...
    CALL add_dim ("height10m",    1,                units  = "m",      &
                                                    ...
                                                    indx   = ABOVESUR10)
!>>ZK #474
    ! Special dimensions used by COSP
    CALL add_dim ("cosplev", Nlr, "height of COSP level above MSL",          &
                                                    units  = "m",            &
                                                    levtyp = 103,            &
                                                    VALUE  = REAL( (/        &
                                                       480,  960, 1440, 1920,&
                                                      ...
                                                      6240, 6720, 7200, 7680,&
                                                      8160, 8640, 9120, 9600,&
                                                     10080,10560,11040,11520,&
                                                     12000,12480,12960,13440,&
                                                     13920,14400,14880,15360,&
                                                     15840,16320,16800,17280,&
                                                     17760,18240,18720,19200 &
                                                    /), KIND=dp),            &
                                                    indx   = COSP_LEVELS)
!<<ZK #474
  ...
  !-----------------------------------------------------------------------------
  SUBROUTINE IO_GET_ATT_INT (ncid, varid, name, ival, lstat)
    INTEGER :: ncid, varid
    CHARACTER(len=*) :: name
    INTEGER :: ival
    LOGICAL, INTENT(out), OPTIONAL :: lstat
