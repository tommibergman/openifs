...
SUBROUTINE UPDTIM(YDGEOMETRY,YDFIELDS,YDMODEL,KSTEP,PTDT,PTSTEP,LDCLUPD,LDUPDECAEC,PI05)
...
USE ECE_CMIP, ONLY : LCMIP6, LCMIP7, LMACV2SP
...
INTEGER(KIND=JPIB)  ,INTENT(IN)    :: KSTEP
...
#include "updclie_aerclim.intfb.h"
...
#endif
#include "cmip_strato_aero.intfb.h"
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP, &
 ...
 &  YDSURF=>YDFIELDS%YRSURF, YDGFL=> YDFIELDS%YRGFL)
...
IF (YDDYNA%LTWOTL) THEN
  ...
  IZT=NINT(PTSTEP*(REAL(KSTEP,JPRD)+0.5_JPRD),JPIB)
ELSE
...
! Fortuin-Langematz O3 CLIMATOLOGY:
  ...
  IF (LMACV2SP) THEN
    ...
    IA0=NCCAA(NINDAT)
    CALL UPDCAL (IJ0,IM0,IA0,NSTADD,IJOUR,IMOIS,IAN,ILMOIS,NULOUT)
    WRITE(NULOUT,'(a,i7)') 'UPDTIM: MACv2SP aerosols for YEAR ', IAN
    CALL SP_SETUP(IAN)
  ENDIF
...
! and  O.Boucher sulphate history (obs 1920-1990, A1B scenario 2000-2100)
  ...
  ! CMIP6/7 stratospheric aerosols optical properties
  IF (LCMIP_STRATAER_CMIP6 .OR. LCMIP_STRATAER_CMIP7) THEN
    CALL CMIP_STRATO_AERO(YDMODEL,YDDYNA)
  ENDIF
...
! update climatology for SO2 oxidation 
   IF(.NOT.LSCMEC) THEN
     IF ( NSO4SCHEME == 2 .AND.LDCLUPD) THEN 
        CALL UPDCLIE_AERCLIM(YDGEOMETRY,YDDYNA,YDMCC,YDGFL,YDMODEL%YRML_GCONF,PTSTEP)
     ENDIF 
  ENDIF
