...
!                            CLIMATOLOGY FILES
!        = .F.   ===> THEY ARE NOT INTERPOLATED
! LMCCIEC_AERCLIM= .T.   ===> THE BOUNDARY CONDITIONS FOR SULFUR OXIDANTS ARE INTERPOLATED IN TIME
!                = .F.   ===> THEY ARE NOT INTERPOLATED
! LMCCICEIC= .T. ===> ICE INITIAL CONDITIONS SENT TO COUPLER AT STARTUP
!           =.F  ===> NO OUTPUT OF ICE

! LFIRSTUPD        ===> TRUE UNTIL UPDCLIE         HAS BEEN CALLED
! LFIRSTUPDCOMPO   ===> TRUE UNTIL UPDCLIE_COMPO   HAS BEEN CALLED
! LFIRSTUPDAERCLIM ===> TRUE UNTIL UPDCLIE_AERCLIM HAS BEEN CALLED
...
! NCLIGC         ===> GRIB CODES OF THE FIELDS TO BE TIME UPDATED FROM CLIMATE FILE                           
! NCLIGC_COMPO   ===> GRIB CODES OF THE FIELDS TO BE TIME UPDATED FROM CLIMATE FILE FOR COMPO                 
! NCLIGC_AERCLIM ===> GRIB CODES OF THE FIELDS TO BE TIME UPDATED FROM CLIMATE FILE FOR AERCLIM (SO2 oxidants)
! NCLIAER        ===> GRIB CODES OF CLIM.AEROSOL FIELDS TO BE TIME UPDATED (not the SO2 oxidants!) [UNUSED]

! NDIFC          ===> DIFFERENCE, IN DAYS, BETWEEN CONSECUTIVE TIMES IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC=.FALSE.)
! NDIFC_COMPO    ===> DIFFERENCE, IN DAYS, BETWEEN CONSECUTIVE TIMES IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC_COMPO=.FALSE.)
! NDIFC_AERCLIM  ===> DIFFERENCE, IN DAYS, BETWEEN CONSECUTIVE TIMES IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC_AERCLIM=.FALSE.)

! NJDCR          ===> JULIAN DAY OF THE NEXT FIELD TO BE READ IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC=.FALSE.)
! NJDCR1         ===> JULIAN DAY OF THE NEXT FIELD TO BE READ IN THE ICMBC FILE (ONLY USED FOR LMCCIEC=.FALSE.)
! NJDCR_COMPO    ===> JULIAN DAY OF THE NEXT FIELD TO BE READ IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC_COMPO=.FALSE.)
! NJDCR1_COMPO   ===> JULIAN DAY OF THE NEXT FIELD TO BE READ IN THE ICMBC FILE (ONLY USED FOR LMCCIEC_COMPO=.FALSE.)  [UNUSED]
! NJDCR_AERCLIM  ===> JULIAN DAY OF THE NEXT FIELD TO BE READ IN THE CLIMATE FILE (ONLY USED FOR LMCCIEC_AERCLIM=.FALSE.)

! NP1            ===> INDEX FOR THE  FIRST FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC=.TRUE.)
! NP2            ===> INDEX FOR THE SECOND FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC=.TRUE.)
! NPAER_1        ===> INDEX FOR THE  FIRST FILE IN TIME INTERPOLATION (ONLY USED FOR LEAERDIU=.TRUE.)
! NPAER_2        ===> INDEX FOR THE SECOND FILE IN TIME INTERPOLATION (ONLY USED FOR LEAERDIU=.TRUE.)
! NPCOMPO_1      ===> INDEX FOR THE  FIRST FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC_COMPO=.TRUE., ELSE FIXED)
! NPCOMPO_2      ===> INDEX FOR THE SECOND FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC_COMPO=.TRUE., ELSE FIXED)
! NPAERCLIM_1    ===> INDEX FOR THE  FIRST FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC_AERCLIM=.TRUE., ELSE FIXED)
! NPAERCLIM_2    ===> INDEX FOR THE SECOND FILE IN TIME INTERPOLATION (ONLY USED FOR LMCCIEC_AERCLIM=.TRUE., ELSE FIXED)
...
! NUNITCAERCLIM  ===> PBOPEN UNIT FOR CLIMATE FILE FOR AERCLIM (SO2 oxidants)
! NUNITC1        ===> PBOPEN UNIT FOR OCEAN BIAS CORRECTION FILE

! CLIMR          ===> HOLDS THE LAST CLIMATE      FILES READ (ONLY USED FOR LMCCEC=.TRUE. .AND. LMCC04=.FALSE.)
! ZLAKE          ===> HOLDS THE LAST LAKE         FILES READ (ONLY USED FOR LEOCLAKE=.TRUE. )
! OCEANBC        ===> HOLDS THE LAST OCEAN BC     FILES READ (ONLY USED FOR LMCCEC=.TRUE. .AND. LMCC04=.FALSE.)
! CLIMRAER       ===> HOLDS THE LAST SO2-OXIDANTS FILES READ (ONLY USED FOR LMCC_COMPO =.TRUE. .AND. NSO4SCHEME == 2)
! CLIMRCOMPO     ===> HOLDS THE LAST COMPO-EMISS  FILES READ (ONLY USED FOR LMCC_COMPO =.TRUE.)
! CLIMRCO2       ===> HOLDS THE LAST NBF CO2      FILES READ (ONLY USED FOR LECO2DIU=.TRUE.)
...
! L2DECV2NEMO    ===> PASS 2D ACV INCREMENT TO NEMO
 ...
 ! To read prescribed oxidants for SO2-SO4 scheme
 INTEGER(KIND=JPIM) :: NCLIGC_AERCLIM(10)
 INTEGER(KIND=JPIM) :: NYSDMP_AERCLIM(10)
 ...
 INTEGER(KIND=JPIM) :: NDIFC_AERCLIM
 INTEGER(KIND=JPIM) :: NJDCR_COMPO
 INTEGER(KIND=JPIM) :: NJDCR1_COMPO
 INTEGER(KIND=JPIM) :: NJDCR_AERCLIM
 ...
 INTEGER(KIND=JPIM) :: NPAERCLIM_1,NPAERCLIM_2
 INTEGER(KIND=JPIM) :: NUNITC
 INTEGER(KIND=JPIM) :: NUNITCM
 INTEGER(KIND=JPIM) :: NUNITCAERCLIM
 ...
 LOGICAL :: LFIRSTUPDCOMPO = .TRUE.
 LOGICAL :: LFIRSTUPDAERCLIM = .TRUE.
 LOGICAL :: LCURR
 LOGICAL :: LGELATO
 LOGICAL :: LMCC01_MSE
 LOGICAL :: LMCC01
...
WRITE(KOUTNO,*) REPEAT(' ',IDEPTHLOC) // 'NYDSMP_COMPO sum = ', SUM(SELF%NYSDMP_COMPO)
