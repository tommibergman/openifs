...
REAL(KIND=JPRB),  INTENT(IN), OPTIONAL :: PPERT(KLON,YDSPP_CONFIG%SM%NRFTOTAL_RADGRID)

! OPTIONAL MACv2SP cloud factor (for indirect aerosol effect)
REAL(KIND=JPRB),  INTENT(IN), OPTIONAL :: MACV2SPCDNC(KLON)
...
IF (YDSPP_CONFIG%LSPP) THEN
  IPN = YDSPP_CONFIG%PPTR%ZRADEFFL
  LLUSE_SPP = IPN > 0
  IF (LLUSE_SPP) THEN
...
CASE(2)
  ...
  DO JL = KIDIA,KFDIA
    ...
    IF (LLUSE_SPP) THEN
      ZNTOT_CM3 = ZNTOT_CM3&
           &    * EXP(PN1%MU(1)  + PN1%XMAG(1) &
           &                         *PPERT(JL,PN1%MP_RADGRID))
    ENDIF

    IF (PRESENT(MACV2SPCDNC)) THEN
        ZNTOT_CM3 = ZNTOT_CM3 * MACV2SPCDNC(JL)
    ENDIF
