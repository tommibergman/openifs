...
SUBROUTINE RADPAR &
 ...
 & PASN,   PEMISF, PLSM   ,&
 & PMU0M,  PGEMU,  PGZ0MCL,&
 & PTVL,   PCO2TYP, PTVH,   PCVL,   PCVH ,PCUR,&
 & PLAILC, PLAIHC, PSOTY, PSOALUVP, PSOALUVD, PSOALNIP, PSOALNID,&
 & PHSTD,  PCI,&
! & PAEROI,&
 ! - UPDATED
 & PNTOP,  PNBAS,  PACPR,&
...
!      F. Vana    11-Sep-2020 Prognostic cloud scheme + cleaning
!-----------------------------------------------------------------------
...
USE YOMCST     , ONLY : RG       ,RLVTT    ,RLSTT    ,RTT  ,RDAY
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PALBF(KLON) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PALUVP(KLON), PALUVD(KLON), PALNIP(KLON), PALNID(KLON)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSOALUVP(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSOALUVD(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSOALNIP(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSOALNID(KLON)
...
INTEGER(KIND=JPIM) :: ITVH(KLON),ITVL(KLON),ISOTY(KLON),ICO2TYP(KLON)
...
REAL(KIND=JPRB) :: ZFRTI(KLON,YDMODEL%YRML_PHY_G%YRDPHY%NTILES),ZCVH(KLON),ZCVL(KLON),ZCUR(KLON)
...
INTEGER(KIND=JPIM) :: IFLAG, JK, JL, JSW
...
!IF (.NOT.LLPJGFB) THEN
  ! Populate these arrays with fixed inputs before they're updated in SURFBC
  ! if LPJ-GUESS is not coupled
  ! ZCVL/ZCVH are updated using the fixed cover fraction modifier or LAI (Beer's
  ! Law)
  ! ZLAIL/ZLAIH are updated using using either fixed look-up table values or
  ! satellite values (LEVLAI)
DO JL=KIDIA,KFDIA
  ZCVL(JL)=PCVL(JL)
  ZCVH(JL)=PCVH(JL)
  ZLAIL(JL)=PLAILC(JL)
...
CALL SURFBC (YDSURF=YSURF,KIDIA=KIDIA,KFDIA=KFDIA,KLON=KLON,KTILES=NTILES,KLEVSN=YDDPHY%NCSNEC,&
 & PTVL=PTVL,PCO2TYP=PCO2TYP,PTVH=PTVH,PSOTY=PSOTY,&
 & PSOALUVP=PSOALUVP, PSOALUVD=PSOALUVD, PSOALNIP=PSOALNIP, PSOALNID=PSOALNID,&
 & PSDOR=ZHSTD,PCVLC=PCVL,PCVHC=PCVH,PCURC=PCUR,&
 ...
 & KTVL=ITVL,KCO2TYP=ICO2TYP,KTVH=ITVH,KSOTY=ISOTY,&
 & PCVL=ZCVL,PCVH=ZCVH,PCUR=ZCUR,PLAIL=ZLAIL,PLAIH=ZLAIH,&
 & PWLMX=ZWLMX,PFRTI=ZFRTI,&
 & PALBF=PALBF,PALUVP=PALUVP,PALUVD=PALUVD,PALNIP=PALNIP,PALNID=PALNID,LRDALB=LRDALB)
...
IF (LERADS) THEN
  ...
  IF (LNEMOLIMALB) THEN
     ...
     IF (ECE_CPL_NEMO_LIM) THEN
        CALL ECE_SI3_GET_ICE_STATE(KSTGLO,KIDIA,KFDIA,ICE_ALBEDO=ZALBICE(KIDIA:KFDIA))
     ELSEIF (ECE_CPL_FESOM_FESIM) THEN
        CALL ECE_FESIM_GET_ICE_STATE(KSTGLO,KIDIA,KFDIA,ICE_ALBEDO=ZALBICE(KIDIA:KFDIA))
     ENDIF
#else
  ...
  CALL SURFRAD(YDSURF=YSURF,KDD=IDD,KMM=IMM,KMON=ILMON,KSECO=ISEC,&
   ...
   & PALCOEFF=PALCOEFF,PCURC=PCUR,PCVH=PCVH,&
...
! routine and make little difference to the random number generator. 
  ...
  ELSEIF (LEPCLD2 .AND. (LNCLIN .OR. (KSTEP /= KSTART))) THEN
    DO JK=1,KLEV
      DO JL=KIDIA,KFDIA
        ZPA(JL,JK) = PA(JL,JK)
      ENDDO
