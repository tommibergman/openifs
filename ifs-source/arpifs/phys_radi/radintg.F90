...
SUBROUTINE RADINTG(YDGEOMETRY, YDMODEL,KLEV  , KMODE,                          &
           ...
           & PAEROTAU,PAEROSSA, PAEROASYM, PAEROTAULW,                         &
           & PDP   , PSPECTRALEMISS,                                           &
           & PMU0  , PQ,                                                       &
...
! PPERTT : (NGPTOT,YDSPP_CONFIG%SM%NRFTOTAL_RADGRID) ; SPP random patterns for radiation
!     ==== OUTPUTS ===
...
!        R. Forbes Nov  2020 Corrected bug in interp of grixbox mean - swapped snow and rain
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROTAU(YDGEOMETRY%YRGEM%NGPTOT,  KLEV,14)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROSSA(YDGEOMETRY%YRGEM%NGPTOT,  KLEV,14)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROASYM(YDGEOMETRY%YRGEM%NGPTOT, KLEV,14)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAEROTAULW(YDGEOMETRY%YRGEM%NGPTOT,KLEV,16)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PPERTT(YDGEOMETRY%YRGEM%NGPTOT, YDMODEL%YRML_GCONF%YRSPP_CONFIG%SM%NRFTOTAL_RADGRID)
...
INTEGER(KIND=JPIM) :: I,   IBEG, IEND,   IB,   IL,  I1, I2, IM, IMD,  IRD,      &
                    & J,   JARP, JIR,    JB,   JK,  JL, JM, JN, JLEV, JRL,      &
                    & JSW, JLW,  JSTGLO, JFLD, JGL, JAERO,                      &
                    & INEXT, IINBEG,  IINEND,   IOUTBEG,  IOUTEND, IFLD, ICNT,  &
                    & IARP,  IFLDSIN, IFLDSOUT, IFLDSTOT, IGPBLKS, IGPBLKS_RAD, &
                    & IGI,   IGIX,    IWIS,     IFLDN,    IFLDX,   IGLGLO,      &
                    & ISUPP
...
INTEGER(KIND=JPIM) :: IMU0,   IAMU0,  IEMISS,  ITS,     ISLM,  IBAS,  ITOP,  &
                    & IFRSOD, IFRTED, IFRSODC, IFRTEDC, IEMIT, ISUDU, IUVDF, &
                    & IPARF,  IPARCF, ITINCF,  IFDIR,   IFDIF, ICDIR, IALD,  &
                    & IALP,   ITI,    IPR,     IQS,     IWV,   ICLC,  ILWA,  &
                    & IIWA,   ISWA,   IRWA,    IRRA,    IDP,   IOZ,   IHTI,  &
                    & IHPR,   IFRSO,  ISWFC,   IFRTH,   ILWFC, IAE,   IAER,  &
                    & IAERO,  IPERT,  IAPRS,   IGELAM,  IGEMU, ICLON, ISLON, &
                    & IECPO3, ICCNL,  ICCNO,   ICO2,    ICH4,  IN2O,  INO2,  &
                    & IC11,   IC12,   IC22,    ICL4,    IFSD,                &
                    & ILWDERIVATIVE, ISWDIRECTBAND, ISWDIFFUSEBAND,          &
                    & IHAMAEROAOD,   IHAMAEROSSA,   IHAMAEROASYM, IHAMAEROAODLW
  
INTEGER(KIND=JPIM) :: IRE_LIQ, IRE_ICE
...
INTEGER(KIND=JPIM) :: IAUX, INLWTEMP
...
ASSOCIATE(&
 ...
 & YDGSGEOM_NB=>YDGEOMETRY%YRGSGEOM_NB, YDHSLMER=>YDMODEL%YRML_DYN%YRSLINT%YRHSLMER,  YDLAP=>YDGEOMETRY%YRLAP, &
 ...
 & YDERDI=>YDMODEL%YRML_PHY_RAD%YRERDI,YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD,YDESWRT=>YDMODEL%YRML_PHY_RAD%YRESWRT, &
 & YDPHY3=>YDMODEL%YRML_PHY_MF%YRPHY3, &
...
ASSOCIATE(&
 & NACTAERO=>YGFL%NACTAERO, NAERO=>YGFL%NAERO, YO3=>YGFL%YO3, &
 ...
 & NCHEM=>YGFL%NCHEM, LCHEM_O3RAD=>YDCHEM%LCHEM_O3RAD)
!     ------------------------------------------------------------------
...
LLSPPRAD=YDSPP_CONFIG%LSPP.AND.(YDSPP_CONFIG%SM%NRFTOTAL_RADGRID >0)


CALL SURF_INQ(YSURF,PREPALB=ZREPALB)
NIMP=0
NOUT=0
ZRII0=RII0*YDESWRT%RADJUST
...
IF (NAERO > 0 .AND. NAERO <= 21 .AND. NAERMACC == 0) THEN
  ...
  IF (LAERVOL) THEN
    ISUPP=ISUPP+3
  ENDIF
  IF (LAERCHEM .AND. .NOT. (LAERSOA .OR. LAERNITRATE .OR. LAERVOL)) THEN
    IRADAER=12
    IRADAER2=11
  ELSE
    IRADAER=12+ISUPP
...
IECPO3 =INDRAD(INEXT,KLEV ,.NOT.LRAYFM.AND.LEPO3RA.AND.(YO3%LGP.OR.(NCHEM>0.AND.LCHEM_O3RAD)))
...
IHAMAEROAOD   =INDRAD(INEXT,KLEV*NTSW,(TRIM(AERO_SCHEME) == "hamm7" .AND. LAERRRTM))
IHAMAEROSSA   =INDRAD(INEXT,KLEV*NTSW,(TRIM(AERO_SCHEME) == "hamm7" .AND. LAERRRTM))
IHAMAEROASYM  =INDRAD(INEXT,KLEV*NTSW,(TRIM(AERO_SCHEME) == "hamm7" .AND. LAERRRTM))
IHAMAEROAODLW   =INDRAD(INEXT,KLEV*INLWTEMP,(TRIM(AERO_SCHEME) == "hamm7" .AND. LAERRRTM))
!write(*,*) "IAERO  ",IAERO
!write(*,*) "IHAMAEROAOD   ",IHAMAEROAOD
!write(*,*) "IHAMAEROSSA   ",IHAMAEROSSA
!write(*,*) "IHAMAEROASYM  ",IHAMAEROASYM


IPERT  =INDRAD(INEXT,YDSPP_CONFIG%SM%NRFTOTAL_RADGRID, LLSPPRAD )
...
!IF (TRIM(AERO_SCHEME) == "hamm7" .AND. NCLOUDACT > 0) THEN
  IRE_LIQ = INDRAD(INEXT,KLEV,(TRIM(AERO_SCHEME) == "hamm7" .AND. NCLOUDACT > 0))
  IRE_ICE = INDRAD(INEXT,KLEV,(TRIM(AERO_SCHEME) == "hamm7" .AND. NCLOUDACT > 0))
!ENDIF
...
IF( LLDEBUG )THEN
  ...
  WRITE(NULOUT,'("RADINTG: IHAMAEROAOD  =",I0)')IHAMAEROAOD
  WRITE(NULOUT,'("RADINTG: IHAMAEROSSA  =",I0)')IHAMAEROSSA
  WRITE(NULOUT,'("RADINTG: IHAMAEROASYM =",I0)')IHAMAEROASYM
  WRITE(NULOUT,'("RADINTG: IHAMAEROASYM =",I0)')IHAMAEROAODLW
  ...
  WRITE(NULOUT,'("RADINTG: IRE_LIQ =",I0)')IRE_LIQ
  WRITE(NULOUT,'("RADINTG: IRE_ICE =",I0)')IRE_ICE
...
!*         3.      PREPARE INPUT ARRAYS
  ...
  ELSE
    ...
    ZRGP(1:IL,ISWA:ISWA+KLEV-1    ,IB) =PQSWP (IBEG:IEND,1:KLEV)
    ZRGP(1:IL,IRWA:IRWA+KLEV-1    ,IB) =PQRAIN(IBEG:IEND,1:KLEV)
  ENDIF
  ...
  ENDIF
  IF( .NOT.LRAYFM.AND.LEPO3RA.AND.(YO3%LGP.OR.(NCHEM>0.AND.LCHEM_O3RAD)) )THEN
...
!-- configuration with prognostic aerosols in radiation
  AERORAD: IF( .NOT.LRAYFM .AND. NACTAERO > 0 .AND. NAERMACC == 0) THEN
...
!-- debug
    ...
    IF (TRIM(AERO_SCHEME) == "hamm7" ) THEN
      ...
      DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROAOD+IAE:IHAMAEROAOD+IAE+KLEV-1,IB) = PAEROTAU(IBEG:IEND,1:KLEV,JAERO)
      ENDDO

      DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROSSA+IAE:IHAMAEROSSA+IAE+KLEV-1,IB) = PAEROSSA(IBEG:IEND,1:KLEV,JAERO)
      ENDDO

      DO JAERO=1,YDMODEL%YRML_PHY_RAD%YRERAD%NTSW
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROASYM+IAE:IHAMAEROASYM+IAE+KLEV-1,IB) = PAEROASYM(IBEG:IEND,1:KLEV,JAERO)
      ENDDO

      DO JAERO=1,16 ! ALaak: Need to be changed to some variable
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IHAMAEROAODLW+IAE:IHAMAEROAODLW+IAE+KLEV-1,IB) = PAEROTAULW(IBEG:IEND,1:KLEV,JAERO)
      ENDDO
  ...
  !   SR 06 2020 LAERCHEM and no nitrate/SOA
      IF (LAERCHEM .AND. .NOT. (LAERSOA .OR. LAERNITRATE .OR. LAERVOL)) THEN
        JAERO=IRADAER2+1
        IAE=(JAERO-1)*KLEV
        ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) =REPSCAER
      ENDIF
  ...
  IF (LLSPPRAD) THEN
    DO JARP=1,YDSPP_CONFIG%SM%NRFTOTAL_RADGRID
      IARP=JARP-1
  ...
  IF (TRIM(AERO_SCHEME) == "hamm7" ) THEN
    IF (NCLOUDACT > 0) THEN
      IF (NSTEP == YDRIP%NSTART) THEN
        ZRGP(1:IL,IRE_LIQ:IRE_LIQ+KLEV-1,IB) = 2.E-6_JPRB
        ZRGP(1:IL,IRE_ICE:IRE_ICE+KLEV-1,IB) = 30.E-6_JPRB
      ELSE
        ZRGP(1:IL,IRE_LIQ:IRE_LIQ+KLEV-1,IB) = PRE_LIQ(IBEG:IEND,1:KLEV)
        ZRGP(1:IL,IRE_ICE:IRE_ICE+KLEV-1,IB) = PRE_ICE(IBEG:IEND,1:KLEV)
      END IF
    END IF
  END IF
...
!$OMP END PARALLEL DO
  ...
  IF( LLDEBUGRW )THEN
    ...
    IULTMP = RESERVE_LUN()
    IF( LLREAD ) THEN
      OPEN(IULTMP,FILE=CL_DEBUGFN,FORM='unformatted',ACTION='READ')
    ELSE
      OPEN(IULTMP,FILE=CL_DEBUGFN,FORM='unformatted',ACTION='WRITE')
    ENDIF
...
!  COMPUTE DISTRIBUTION OF CLIMATOLOGICAL AEROSOLS AND OZONE DIRECTLY ON THE REDUCED RADIATION GRID
  ...
  IF(.NOT.LRAYFM) THEN
    ...
    IF( LEPO3RA.AND.(YO3%LGP.OR.(NCHEM>0.AND.LCHEM_O3RAD)) )THEN
    ...
    DO JL=1,IL
      IF( ZDGEMU(JL,IB) /= ZDGEMU(I1,IB) .OR. JL == IL )THEN
        ...
        CALL  RADACT ( YDEAERD,YDERAD,YDMODEL%YRML_PHY_AER%YREAERSNK,YDRIP, &
         & YDSPP_CONFIG, I1 ,I2 ,NRPROMA ,KLEV,&
...
!*         4.2     CALL TO ACTUAL RADIATION SCHEME
  ...
  ELSE
    ...
    CALL RADIATION_SCHEME &
     & (YDGEOMETRY, YDMODEL, 1, IL, NRPROMA, KLEV, IRADAER, &
     &  ZRII0, &
     ...
     &  ZRGP(1,ISwDiffuseBand,IB), ZRGP(1,ISwDirectBand,IB),&
     ! OPTIONAL ARGUMENTS
     &  PAEROM7_TAU=ZRGP(1,IHAMAEROAOD,IB),  PAEROM7_SSA=ZRGP(1,IHAMAEROSSA,IB), PAEROM7_ASYM=ZRGP(1,IHAMAEROASYM,IB), &
     &  PAEROM7_TAULW=ZRGP(1,IHAMAEROAODLW,IB),  &
     &  PRE_LIQ=ZRGP(1,IRE_LIQ,IB), PRE_ICE=ZRGP(1,IRE_ICE,IB),&
...
!-- reconstruct Tegen aer LW OD
    IAE=1
    ZTAUAER(1:IL,IAE) =&
   & YDESWRT%RAER(IAE,1)*PAER(IBEG:IEND,1,JK)+YDESWRT%RAER(IAE,2)*PAER(IBEG:IEND,2,JK)&
   & +YDESWRT%RAER(IAE,3)*PAER(IBEG:IEND,3,JK)+YDESWRT%RAER(IAE,4)*PAER(IBEG:IEND,4,JK)&
   & +YDESWRT%RAER(IAE,5)*PAER(IBEG:IEND,5,JK)+YDESWRT%RAER(IAE,6)*PAER(IBEG:IEND,6,JK)  
...
!-- for debugging only
    ...
    CALL SATUR (1, IL, IL  , 1, KLEV, YDMODEL%YRML_PHY_SLIN%YREPHLI%LPHYLIN, &
