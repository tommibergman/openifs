...
SUBROUTINE SURFBC_CTL(KIDIA, KFDIA, KLON, KTILES,KLEVSN, &
 ...
 & PSDOR, PCVLC, PCVHC, PCURC, PLAILC, PLAIHC, PLAILI, PLAIHI, &
...
REAL(KIND=JPRB),    INTENT(OUT)  :: PALUVP(:),PALUVD(:)
REAL(KIND=JPRB),    INTENT(OUT)  :: PALNIP(:),PALNID(:)
...
ASSOCIATE(LEFLAKE=>YDFLAKE%LEFLAKE, RH_ICE_MIN_FLK=>YDFLAKE%RH_ICE_MIN_FLK, &
 ...
 & RVCOV=>YDVEG%RVCOV, RVLAI=>YDVEG%RVLAI, RVVEGALB=>YDVEG%RVVEGALB)
...
IF (.NOT. LRDALB) THEN
  DO JL=KIDIA,KFDIA
    IF (LDLAND(JL)) THEN 
      ZPCVL=PCVL(JL)
      ZPCVH=PCVH(JL)
      ZPCVB=1.0_JPRB-ZPCVL-ZPCVH
      PALUVP(JL)=RVVEGALB(KTVL(JL),1)*ZPCVL+&
        & RVVEGALB(KTVH(JL),1)*ZPCVH+&
        & PSOALUVP(JL)*ZPCVB
      PALUVD(JL)=RVVEGALB(KTVL(JL),2)*ZPCVL+&
        & RVVEGALB(KTVH(JL),2)*ZPCVH+&
        & PSOALUVD(JL)*ZPCVB
      PALNIP(JL)=RVVEGALB(KTVL(JL),3)*ZPCVL+&
        & RVVEGALB(KTVH(JL),3)*ZPCVH+&
        & PSOALNIP(JL)*ZPCVB
      PALNID(JL)=RVVEGALB(KTVL(JL),4)*ZPCVL+&
        & RVVEGALB(KTVH(JL),4)*ZPCVH+&
        & PSOALNID(JL)*ZPCVB
      PALBF(JL)=0.459760*PALUVD(JL)+0.540240*PALNID(JL)
    ENDIF
  ENDDO
ENDIF
