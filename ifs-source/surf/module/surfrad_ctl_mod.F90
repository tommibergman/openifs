...
USE SURFECE, ONLY : SURFECE_GET_LANDICE, ECE_LANDICE_THRESH, &
 & ECE_CPL_NEMO_LIM, ECE_CPL_FESOM_FESIM
...
IF (ZLANDICE(JL) > ECE_LANDICE_THRESH) THEN
  ZEMISSTI(:,3) = REMISS_SNOW(1:NLWEMISS)
  ZEMISSTI(:,4) = REMISS_SNOW(1:NLWEMISS)
  ZEMISSTI(:,6) = REMISS_SNOW(1:NLWEMISS)
  ZEMISSTI(:,8) = REMISS_SNOW(1:NLWEMISS)
END IF
...
! Ice sheet coupling
    IF (ZLANDICE(JL) > ECE_LANDICE_THRESH) THEN
      ZADTI3 = 0.6_JPRB ; ZAPTI3 = 0.6_JPRB
      ZADTI4 = 0.6_JPRB ; ZAPTI4 = 0.6_JPRB
      ZADTI6 = 0.6_JPRB ; ZAPTI6 = 0.6_JPRB
      ZADTI8 = 0.6_JPRB ; ZAPTI8 = 0.6_JPRB
    END IF
