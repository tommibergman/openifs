...
USE ECEARTH, ONLY : ECE_CPL_NEMO_LIM,  ECE_CPL_FESOM_FESIM, ECE_CPL_STAGE_OCE_SND
USE CPLNG2,  ONLY : CPLNG2_EXCHANGE
#endif
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 ...
 & YDRIP=>YDMODEL%YRML_GCONF%YRRIP,YDDYNA=>YDMODEL%YRML_DYN%YRDYNA) ! YDDYNA added for (EC-Earth) coupling
...
! CALL UPDTIM(YDGEOMETRY,YDFIELDS,YDMODEL,JSTEP,TDT,TSTEP,.FALSE.)
  CALL UPDTIM(YDGEOMETRY,YDFIELDS,YDMODEL,INT(JSTEP,KIND=JPIB),TDT,TSTEP,.FALSE.) ! Update for EC-Earth
...
!  IF (ECE_CPL_NEMO_LIM .OR. ECE_CPL_FESOM_FESIM) CALL CPLNG2_EXCHANGE(INT(TSTEP,KIND=JPIM),ECE_CPL_STAGE_OCE_SND,YDDYNA,YDRIP)
  CALL CPLNG2_EXCHANGE(INT(TSTEP,KIND=JPIM),ECE_CPL_STAGE_OCE_SND,YDDYNA,YDRIP)
#endif
